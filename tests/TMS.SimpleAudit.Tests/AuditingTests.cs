...
public class AuditingTests
{
    ...
    public async Task Should_Add_Audit_Trail_On_Insert(bool explicitInclude)
    {
        ...
        using (var wrapper = contextFactory.Create())
        {
            _auditConfig.Config(wrapper, explicitInclude);

        // act
            (auditableRow, customAuditInfo) = await DataSetup.SeedAsync(wrapper);
        }
        ...
        using (var dbContext = contextFactory.Create())
        {
            var columnChanges = await Assertion.AssertTrailsAndGetColumnChangesAsync(
                dbContext,
                auditableRow.Id,
                _auditConfig.AuditableTableAlias,
                AuditAction.Added,
                customAuditInfo,
    ...
    public async Task Should_Add_Audit_Trail_On_Update(bool explicitInclude)
    {
        ...
        using (var wrapper = contextFactory.Create())
        {
            ...
            await wrapper.SaveChangesAsync(customAuditInfo);
        }
    ...
    public async Task Should_RollBack_Audit_Trails_On_Error(bool explicitInclude)
    {
        ...
        using (var dbContext = contextFactory.Create())
        {
            ...
            // trigger primary key violation error
    ...
    [InlineData(true)]
    [InlineData(false)]
    public async Task Should_Add_Audit_Trail_On_Insert_Retry(bool explicitInclude)
    {
        // arrange
        CustomAuditInfo? customAuditInfo;
        AuditableTableModel? auditableRow;
        ...
        using (var wrapper = contextFactory.Create())
        {
            ...
            var mockedWrapper = new Mock<Audita>();

            mockedWrapper.Setup( c => c.)
